...
export default function Sidebar({ filters, setFilters }) {
  ...
  const availableCategories = [
    ...
    ...new Set(productsData.map((product) => 
      i18n.language === "ar" && product.category_ar ? product.category_ar : product.category_en
    )),
  ];
  ...
  const handleCategoryClick = (category) => {
    // Get the English category name for filter if current language is Arabic
    const categoryForFilter = category === "All" ? "" : category;
  ...
  const isCategoryActive = (category) => {
    if (category === "All") return filters.category === "";
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between text-sm text-gray-600 mb-2">
                ...
                <span className="bg-amber-50 text-amber-700 px-3 py-1 rounded-full text-sm font-medium">
                  {priceRange.min} {t("sidebar.currency")} - {priceRange.max === Infinity ? maxPrice : priceRange.max} {t("sidebar.currency")}
                </span>
              </div>
              ...
              <div className="flex justify-between text-xs text-gray-500 mt-2">
                <span>
                  {minPrice} {t("sidebar.currency")}
                </span>
                <span>
                  {maxPrice} {t("sidebar.currency")}
                </span>
              </div>
            </div>
            ...
            <ul className="space-y-4">
              {popularProducts.length > 0 ? (
                popularProducts.map((product, index) => (
                  <li
                    key={index}
                    className={`flex items-center p-2 hover:bg-white rounded-lg transition-all duration-300 cursor-pointer group ${i18n.language === "ar" ? "space-x-reverse" : "space-x-3"}`}>
